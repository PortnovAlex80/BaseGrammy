...
> ⚠️ **Статус продукта (аудит 2026-08-26):** v2 — это **incomplete product slice**,
> а не готовый продукт. Таблица фаз ниже отражает прогресс *каркаса* (scaffold),
> но не product readiness: реальный пользовательский путь fresh-install → урок →
> завершение не работает (production обходит `SessionEngine`, сессия создаётся
> без пула, `PackImporter` и аудио-стек содержат runtime `TODO()`). Актуальный
> статус и план стабилизации: [`docs/architecture/REFACTORING_PLAN_2026-08-26.md`](../architecture/REFACTORING_PLAN_2026-08-26.md).
