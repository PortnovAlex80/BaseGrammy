...
- **FR-2.3.1** Система должна создавать бэкап сразу после успешного прохождения упражнения (под-урока).
- **FR-2.3.2** Основной бэкап сохраняется в `Downloads/BaseGrammy/backup_latest/` (при наличии могут существовать архивные папки `backup_YYYY-MM-DD_HH-MM-SS/`).
- **FR-2.3.3** Бэкап должен включать: `mastery.yaml`, `progress.yaml`, `streak_<languageId>.yaml`, `profile.yaml`
...
- **FR-3.4.5** Система должна использовать ВСЕ доступные карточки урока, а не только первые 150

### 3.5 Циклическое отображение подуроков
- **FR-3.5.1** Подуроки должны отображаться блоками по 15 (1-15, 16-30, 31-45 и т.д.)
- **FR-3.5.2** После завершения 15 подуроков должен открываться следующий блок
- **FR-3.5.3** Номера подуроков должны отображаться согласно текущему блоку
- **FR-3.5.4** Прогресс "Exercise N of M" должен показываться в рамках текущего блока из 15
- **FR-3.5.5** Урок должен считаться завершённым после первых 15 подуроков
- **FR-3.5.6** Пользователь может продолжать проходить дополнительные блоки после завершения урока
- **FR-3.5.7** Если карточек меньше 15 подуроков, должно показываться соответствующее количество
- **FR-3.5.8** После завершения всех подуроков плитки тренировок не отображаются
