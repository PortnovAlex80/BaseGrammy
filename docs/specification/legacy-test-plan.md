...
## НОВОЕ: Циклические подуроки (Cyclic Sublessons)

### MixedReviewSchedulerTest - Циклические подуроки [ДОБАВЛЕНО]
**Приоритет:** P0 - КРИТИЧЕСКИЙ
**Файл:** `app/src/test/java/com/alexpo/grammermate/data/MixedReviewSchedulerTest.kt`
**Дата добавления:** 2026-01-18
**Цель:** Тестирование использования всех карточек и циклического отображения

#### Новые тесты:

##### 1. Использование всех карточек
```kotlin
@Test fun build_usesAllCardsNotJustFirst150()
```
**Описание:** Проверяет что система использует все 300 карточек, а не только первые 150
**Покрываемые требования:** FR-3.4.5
**Ожидаемый результат:** При уроке из 300 карточек все карточки используются в подуроках

##### 2. Создание циклических подуроков
```kotlin
@Test fun build_createsCyclicSublessons()
```
**Описание:** Проверяет что создаются подуроки в циклах по 15
**Покрываемые требования:** FR-3.5.1, FR-3.5.2
**Ожидаемый результат:** 200 карточек создают 20 подуроков (15 в первом цикле, 5 во втором)

##### 3. Обработка менее 15 подуроков
```kotlin
@Test fun build_handlesLessThan15Sublessons()
```
**Описание:** Проверяет корректную обработку уроков с малым количеством карточек
**Покрываемые требования:** FR-3.5.7
**Ожидаемый результат:** 80 карточек создают 8 подуроков (не дополняются до 15)

**Метрики успеха:**
- ✅ Все карточки используются (не только первые 150)
- ✅ Подуроки создаются корректно независимо от количества карточек
- ✅ Циклы по 15 формируются правильно
...
5. Запустить новые тесты для циклических подуроков
