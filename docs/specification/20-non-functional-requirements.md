...
| Store data SHOULD be cached in memory after first load | SHOULD | Partially implemented. MasteryStore, HiddenCardStore, BadSentenceStore, and VocabProgressStore already use lazy in-memory caching. LessonStore, VerbDrillStore, WordMasteryStore, ProgressStore, DrillProgressStore, and AppConfigStore still read from disk on every call. Future optimization: add lazy-loading cache with write-through to the remaining stores. |
