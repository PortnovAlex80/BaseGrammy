...
| 10 | **Significant** | `DailyPracticeCoordinator.resetState()` was wiping `dailyCursor` to defaults (sentenceOffset=0, currentLessonIndex=0) when called from `selectLesson()`, `selectLanguage()`, or `importLessonPack()`. This violated the Independence Principle (09#9.1) -- daily cursor should be independent of lesson roadmap browsing. **RESOLVED by TASK-070 Fix 3**: `resetState()` now uses `_state.update { it.copy(dailySession = DailySessionState()) }` which preserves `dailyCursor`. A new `resetAllDailyState()` method does the full wipe and is only called from "Reset all progress" in Settings. Spec 09#9.8.7 updated. | `DailyPracticeCoordinator.kt:resetState()`, `DailyPracticeCoordinator.kt:resetAllDailyState()` | RESOLVED (TASK-070 Fix 3) |
