...
object MultilingualStoryParser {
     ...
     * Supported languages: it (Italian), en (English), ru (Russian)
     */
    private val languagePattern = Regex("""\{(it|en|ru)\}(.+?)\{/\1\}""", RegexOption.DOT_MATCHES_ALL)

    /**
     * Detect the language of a text sample by character analysis and word patterns.
     * Returns "ru", "en", "it", or defaultLanguageId.
     */
    private fun detectLanguage(text: String, defaultLanguageId: String): String {
        ...
        val total = ruChars + enChars + itChars
        if (total == 0) {
        ...
        Log.d(TAG, "detectLanguage: ruChars=$ruChars ruRatio=$ruRatio, enChars=$enChars enRatio=$enRatio, itChars=$itChars itRatio=$itRatio")
        Log.d(TAG, "detectLanguage: italianWords=$italianWordMatches, englishWords=$englishWordMatches")
