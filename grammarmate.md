...
- **FR-11** При верном ответе должен и при голосовом вводе автоматически переходим на следующее предложение с активацией голосового ввода
- **FR-12** После 3 неверных попыток по одной карточке: показать подсказку (эталон/варианты) и ожидать действия пользователя - активна иконка голосового ввода и с клавиатуры.
- **FR-13** После 3 неверных попыток по одной карточке счетчик неверных попыток сбрасывается
...
- **FR-36** При загрузке урока сравнить название урока - если не отличается, то обновить с удалением старого урока с тем же названием
- **FR-37** При нажатии на шестеренку, режим приложения переходит в паузу.
- **FR-38** При переходе на главный экран тренажера, режим приложения переходит в режим готовности ввода перевода (голосовой и текстовый ввод активен).
- **FR-38** Если уроков нет, то приложение блокирует текстовый и головосой ввод с надписью Карточек нет
